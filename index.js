...
async function getOrCreateThread(sender_psid) {
  if (userThreads[sender_psid]) {
    ...
    // If the thread has more than 10 messages, create a new thread
    if (messages.data.length >= 10) {
      console.log("Creating new thread for user:", sender_psid);
      const newThread = await openai.beta.threads.create({}, {
        headers: { "OpenAI-Beta": "assistants=v2" }
      });
      userThreads[sender_psid] = newThread.id; // Update with the new Thread ID
      return newThread.id;
    }

    // If not more than 10 messages, use the existing thread
    return thread_id;
  } else {
...
// Function to use ChatGPT via OpenAI Assistant API
async function getChatGPTResponse(sender_psid, userMessage) {
  try {
    ...
    // Retrieve all messages in the Thread to count them
    const messages = await openai.beta.threads.messages.list(thread_id, {
      headers: { "OpenAI-Beta": "assistants=v2" }
    });
    ...
    // Run the Assistant to get a response using assistant_id from environment variables
    const runResponse = await openai.beta.threads.runs.create(
      thread_id,
      {
        assistant_id: process.env.OPENAI_ASSISTANT_ID,
        // Instructions for the Assistant on how to operate
...
// Clean response: handle duplicate links and trim whitespace
function cleanResponse(text) {
  if (!text) return "ขออภัย ฉันไม่สามารถตอบคำถามได้ในขณะนี้";
  
  // The following lines that remove references have been commented out as per your request.
  // text = text 
  ...
  text = text.replace(/\[([^\]]+)\]\((https?:\/\/[^\)]+)\)/g, "$2"); // Handle URL markdown
...
// Webhook endpoint for receiving messages from Facebook Messenger
...
// Function to send messages back to the user in Messenger
